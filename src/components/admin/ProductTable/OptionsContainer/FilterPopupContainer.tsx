  ...
  const addFilter = () => {
    setFilters((current) => {
      ...
      const notFiltered = columns.filter((column) => {
        return current.some((filteredCol) => filteredCol.column === column)
          ? false
          : true;
      });
      if (!!notFiltered[0])
        return [...current, { column: notFiltered[0], compare: "", type: "=" }];
      else return current;
    });
  };
    ...
    >
      ...
      <div className="border-t-[1px] mr-1 flex min-h-[2.5rem] min-w-full flex-col border-slate-200">
        <div className="py-2">
          <button
            disabled={
              filters.length === Object.keys(columnsData).length ? true : false
            }
