...
export default function ProductImage({
  ...
  useEffect(() => {
    fetchImage(filepath).then(
      (blob) => !(blob instanceof Error) && setURL(URL.createObjectURL(blob))
    );
  }, [filepath]);
      ...
      toggleMainImage(imageId, productId, isMain)
        .then((imageData) => {
          !(imageData instanceof Error) &&
            setImages(imageData.sort((a, b) => a.primary - b.primary));
        })
        .catch((error) => {
  ...
  if (url) {
    return (
      <div
        className={`${
          !!isMain ? "rounded-sm ring-[6px] ring-blue-500 ring-opacity-60" : ""
        } 
